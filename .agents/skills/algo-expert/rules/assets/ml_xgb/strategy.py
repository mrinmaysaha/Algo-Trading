"""
ML XGBoost - dual-mode strategy.

Loads a pre-trained XGBClassifier (see train.py) and uses its prediction
to generate entry/exit signals.

Workflow:
    1. python train.py --symbol RELIANCE
    2. python strategy.py --mode backtest
    3. python strategy.py --mode live
"""
...
PRODUCT         = "MIS"
LOT_SIZE        = 1
STRATEGY_NAME   = os.getenv("STRATEGY_NAME", "ml_xgb")
DATA_SOURCE     = os.getenv("DATA_SOURCE", "api")
RISK_PER_TRADE  = 0.005
MAX_SIZE_PCT    = 0.50
MODEL_PATH      = _HERE / f"{SYMBOL}_xgb.pkl"
PROB_THRESHOLD  = 0.55
...
def signals(df, bundle):
    ...
    proba_full = pd.Series(proba, index=X.index).reindex(df.index)
...
def run_backtest():
    ...
    bundle = load_model()
    log.info("Model trained %s walk-forward acc=%.4f",
...
def run_live():
    ...
    def on_bar_close(df):
        ...
        if entries.iloc[-2] and state["position"] is None:
            ...
            log.info("XGB ENTRY %s @ %.2f", df.index[-2], ltp)
            ...
            log.info("XGB EXIT %s @ %.2f", df.index[-2], ltp)
            try:
