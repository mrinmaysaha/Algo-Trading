"""
RSI Mean Reversion - dual-mode strategy.
...
PRODUCT         = "MIS"
LOT_SIZE        = 1
STRATEGY_NAME   = os.getenv("STRATEGY_NAME", "rsi_meanrev")
...
RISK = RiskConfig(sl_pct=0.015, tp_pct=0.03, trail_pct=0.012, time_exit_min=180)
INIT_CASH       = 1_000_000
LOOKBACK_DAYS   = 365 * 2
...
def signals(df):
    ...
    entries = ((rsi_prev <= OVERSOLD) & (rsi > OVERSOLD)).fillna(False).astype(bool)
    exits   = ((rsi_prev >= OVERBOUGHT) & (rsi < OVERBOUGHT)).fillna(False).astype(bool)
...
def run_backtest():
    ...
    df = fetch_backtest_data(client, SYMBOL, EXCHANGE, INTERVAL,
                             ...
                             source=DATA_SOURCE)
    if df is None or len(df) < RSI_PERIOD * 3:
    ...
    log.info("Trades CSV: %s", out / f"{SYMBOL}_trades.csv")
...
def run_live():
    ...
    log.info("Cost: %s slip=%.4f%%", COSTS.label, COSTS.slippage*100)
    ...
    if resumed is not None:
        log.info("Resuming from state.db")
...
def _sh(signum, frame):
    log.info("signal %d - shutting down", signum); stop_event.set()
...
def main():
    ...
    args = p.parse_args()
    if args.mode == "backtest": run_backtest()
    else: run_live()
