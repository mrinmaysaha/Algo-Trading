"""
Supertrend - dual-mode strategy.

Buy when supertrend direction flips from down to up.
Sell when supertrend direction flips from up to down.

Supertrend always comes from openalgo.ta - talib doesn't have it.
"""
...
INTERVAL        = "5m"
PRODUCT         = "MIS"        # adjust to NRML for futures
LOT_SIZE        = 1            # 1 for index proxy; 65 for NIFTY futures
STRATEGY_NAME   = os.getenv("STRATEGY_NAME", "supertrend")
DATA_SOURCE     = os.getenv("DATA_SOURCE", "api")
RISK_PER_TRADE  = 0.005
MAX_SIZE_PCT    = 0.50
ATR_PERIOD      = 10
ST_MULTIPLIER   = 3.0
INDICATOR_LIB   = "openalgo"   # supertrend always openalgo regardless
EXECUTION_TYPE  = "eoc"
POLL_INTERVAL_SEC = 15
RISK = RiskConfig(sl_pct=0.012, tp_pct=None, trail_pct=0.015, time_exit_min=None)
INIT_CASH       = 1_000_000
LOOKBACK_DAYS   = 365 * 2
...
def signals(df):
    ...
    _, direction = ind.supertrend(df["high"], df["low"], df["close"], ATR_PERIOD, ST_MULTIPLIER)
    direction = pd.Series(direction, index=df.index)
    # direction = 1 (up) / -1 (down). Detect flips.
    prev = direction.shift(1)
    entries = ((prev <= 0) & (direction > 0)).fillna(False).astype(bool)
    exits   = ((prev > 0) & (direction <= 0)).fillna(False).astype(bool)
