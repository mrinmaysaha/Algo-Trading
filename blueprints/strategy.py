...
webhook_strategy_bp = Blueprint("webhook_strategy_bp", __name__, url_prefix="/webhook-strategy")
strategy_bp = webhook_strategy_bp
...
def process_legacy_webhook(strategy, incoming_req):
    """Process an inbound TradingView alert for a legacy webhook strategy."""
    try:
        if not strategy.is_active:
            return jsonify({"error": "Strategy is inactive"}), 400

        data = incoming_req.get_json(silent=True) if hasattr(incoming_req, "get_json") else None
        if not data:
...
@webhook_strategy_bp.route("/webhook/<webhook_id>", methods=["POST"])
...
def webhook(webhook_id):
    ...
    return process_legacy_webhook(strategy, request)
