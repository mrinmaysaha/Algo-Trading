...
import type { Strategy, StrategySymbolMapping, SymbolSearchResult } from '@/types/webhookStrategy'
import { EXCHANGES, getProductTypes } from '@/types/webhookStrategy'
...
export default function ConfigureSymbols() {
  ...
  const searchSymbols = useCallback(
    async (query: string) => {
      ...
      try {
        ...
        const results = await webhookStrategyApi.searchSymbols(query, exchange || undefined)
  ...
  const handleSingleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true)
      const response = await webhookStrategyApi.addSymbolMapping(Number(strategyId), {
  ...
  const handleBulkSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true)
      const response = await webhookStrategyApi.addBulkSymbols(Number(strategyId), csvData)
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <Button variant="ghost" asChild>
        <Link to={`/webhook-strategy/${strategyId}`}>
