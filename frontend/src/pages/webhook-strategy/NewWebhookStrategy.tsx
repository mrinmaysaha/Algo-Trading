...
import { PLATFORMS, TRADING_MODES } from '@/types/webhookStrategy'
...
export default function NewStrategy() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      const response = await webhookStrategyApi.createStrategy({
        name: formData.name,
      ...
      if (response.status === 'success') {
        ...
        navigate(`/webhook-strategy/${response.data?.strategy_id}`)
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              <Button
                ...
                variant="outline"
                className="flex-1"
                onClick={() => navigate('/webhook-strategy')}
              >
                Cancel
              </Button>
