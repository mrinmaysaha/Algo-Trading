...
import { webhookStrategyApi } from '@/api/webhookStrategy'
...
import type { Strategy, StrategySymbolMapping } from '@/types/webhookStrategy'
...
export default function ViewStrategy() {
  ...
  const fetchStrategy = async () => {
    ...
    try {
      setLoading(true)
      const data = await webhookStrategyApi.getStrategy(Number(strategyId))
      setStrategy(data.strategy)
      setMappings(data.mappings || [])
    } catch (_error) {
      showToast.error('Failed to load strategy', 'strategy')
      navigate('/webhook-strategy')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleToggle = async () => {
    ...
    try {
      setToggling(true)
      const response = await webhookStrategyApi.toggleStrategy(strategy.id)
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true)
      const response = await webhookStrategyApi.deleteStrategy(strategy.id)
      if (response.status === 'success') {
  ...
  const handleDeleteMapping = async (mappingId: number) => {
    if (!strategy) return
    try {
      const response = await webhookStrategyApi.deleteSymbolMapping(strategy.id, mappingId)
      if (response.status === 'success') {
  ...
  "action": "BUY"
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button asChild>
            <Link to={`/webhook-strategy/${strategy.id}/configure`}>
          ...
          {mappings.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <Button variant="link" asChild>
                <Link to={`/webhook-strategy/${strategy.id}/configure`}>Add your first symbol</Link>
              </Button>
            </div>
          ) : (
            <Table>
