...
"""
================================================================================
OPENALGO PRODUCTION STRATEGY: BTC DONCHIAN ASYMMETRIC TREND RIDER (PERP)
================================================================================
Target Symbol   : BTCUSDFUT (Delta Exchange: BTCUSD)
Exchange        : CRYPTO
...
logger = logging.getLogger("BTC_Trend_Rider")

# Strategy Parameters
STRATEGY_NAME = "BTC_Donchian_Trend_Rider"
SYMBOL = "BTCUSDFUT"
DELTA_SYMBOL = "BTCUSD"
EXCHANGE = "CRYPTO"
PRODUCT = "NRML"
...
DONCHIAN_EXIT_PERIOD = 20      # 20 hours trailing exit
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 72-bar Donchian Entry Channel (shifted by 1 to prevent lookahead)
    ...
    atr = np.zeros(n)
    atr[1:] = pd.Series(tr).rolling(ATR_PERIOD).mean().fillna(100.0).values
    df['atr'] = atr
    
    return df


def run_backtest(data_path: str = "data/crypto/BTCUSD_1h_1year.parquet", capital: float = ACCOUNT_CAPITAL_USD, leverage: float = DEFAULT_LEVERAGE):
    """Executes a 1-year deterministic backtest with exact Delta Exchange fee & GST modeling."""
    logger.info("=" * 80)
    logger.info(f"RUNNING BTC DONCHIAN ASYMMETRIC TREND BACKTEST ({leverage}x LEVERAGE)")
    ...
    for i in range(200, n):
        ...
        if in_pos:
            ...
            if exit_trade:
                ...
                # Fees: Entry Maker, Exit Taker + 18% GST
        ...
        # Check Entries
        if not in_pos:
            # Bullish Breakout: Close > 72-bar High + Close > 200 EMA
            ...
            # Bearish Breakdown: Close < 72-bar Low + Close < 200 EMA
...
def log_15min_status_heartbeat(client, last_candle: dict, indicators: dict, active_pos: dict):
    ...
    Logs indicators, ATR, regime alignment, and active position metrics.
    """
    ...
    logger.info(f"  • Donchian 72 High : ${indicators.get('donchian_high', 0.0):,.2f} | Low: ${indicators.get('donchian_low', 0.0):,.2f}")
    logger.info(f"  • Exit Channel 20  : High = ${indicators.get('exit_high', 0.0):,.2f} | Low = ${indicators.get('exit_low', 0.0):,.2f}")
    ...
    if qty != 0:
        ...
        logger.info(f"  • ACTIVE POSITION  : FLAT (Scanning for 72-bar breakout)")
    logger.info("=" * 80)
...
def run_live(api_key: str = None, host: str = None, dry_run: bool = False):
    ...
    while True:
        try:
            ...
            # In dry-run or if API returned empty, fallback to local parquet
            data_file = "data/crypto/BTCUSD_1h_1year.parquet"
            if df is None or (isinstance(df, pd.DataFrame) and df.empty):
                if os.path.exists(data_file):
                    df = pd.read_parquet(data_file).tail(120)
                elif os.path.exists(f"/app/{data_file}"):
                    df = pd.read_parquet(f"/app/{data_file}").tail(120)
                
            if df is not None and isinstance(df, pd.DataFrame) and len(df) >= 75:
                ...
                curr_atr = float(last.get('atr', 100.0))
                ...
                if client and not dry_run:
                    ...
                    # 2. Check Breakout Entries if FLAT
                        ...
                        if curr_close > don_high and curr_close > ema200:
                            ...
                            # 1 BTC contract = 0.001 BTC. Loss per contract = initial_sl_dist * 0.001
                            loss_per_contract = max(1.0, initial_sl_dist * 0.001)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BTC Donchian Asymmetric Trend Rider")
    parser.add_argument("--mode", choices=["backtest", "live"], default=os.getenv("MODE", "live"), help="Execution mode")
    parser.add_argument("--data", default="data/crypto/BTCUSD_1h_1year.parquet", help="Backtest data path")
