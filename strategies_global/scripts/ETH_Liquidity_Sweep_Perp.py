...
"""
================================================================================
OPENALGO PRODUCTION STRATEGY: ETH LIQUIDITY SWEEP (SFP) PERPETUAL FUTURES
================================================================================
Target Symbol   : ETHUSDFUT (Delta Exchange: ETHUSD Perpetual)
Exchange        : CRYPTO
...
logger = logging.getLogger("ETH_Liquidity_Sweep")

# ------------------------------------------------------------------------------
# 1. PARAMETERS & CONFIGURATION
# ------------------------------------------------------------------------------
STRATEGY_NAME = "ETH_Liquidity_Sweep"
SYMBOL = "ETHUSDFUT"
DELTA_SYMBOL = "ETHUSD"
EXCHANGE = "CRYPTO"
PRODUCT = "NRML"
...
SWING_LOOKBACK_BARS = int(os.getenv("ETH_SWING_LOOKBACK", "48"))       # 48 1H bars = 2-day rolling swing
MIN_REJECTION_WICK_RATIO = float(os.getenv("ETH_WICK_RATIO", "0.35")) # Confirmed 35% rejection wick
MIN_VOLUME_RATIO = float(os.getenv("ETH_MIN_VOLUME_RATIO", "0.75"))
MIN_SWEEP_DEPTH_PCT = float(os.getenv("ETH_MIN_SWEEP_DEPTH", "0.0003"))
ENABLE_EMA_FILTER = True                                               # Enforce Triple 4H Trend Alignment
EMA_FAST_SPAN = int(os.getenv("ETH_EMA_FAST", "20"))
EMA_MID_SPAN = int(os.getenv("ETH_EMA_MID", "50"))
EMA_SLOW_SPAN = int(os.getenv("ETH_EMA_SLOW", "100"))
TARGET_RR = float(os.getenv("ETH_TARGET_RR", "1.8"))                   # 1:1.8 Risk-to-Reward (58.8% to 64.7% win rate)
EXECUTION_ORDER_TYPE = os.getenv("ETH_ORDER_TYPE", "MARKET")

MAX_LEVERAGE = float(os.getenv("ETH_MAX_LEVERAGE", "12.0"))             # Strict 12x leverage cap
MIN_RISK_DIST_PCT = float(os.getenv("ETH_MIN_RISK_DIST", "0.0020"))     # 0.20% minimum stop distance
MAX_HOLD_BARS = int(os.getenv("ETH_MAX_HOLD_BARS", "72"))               # 72 hours max hold
CAPITAL_BASE_INR = float(os.getenv("CAPITAL_BASE_INR", os.getenv("ETH_FUT_CAPITAL_INR", "10000.0")))
USD_INR_RATE = float(os.getenv("USD_INR_RATE", "88.0"))
RISK_PER_TRADE_PCT = float(os.getenv("ETH_RISK_PER_TRADE", "0.06"))     # 6% capital risk per trade

def get_state_file_path() -> Path:
    base_dir = Path(__file__).resolve().parent.parent / "data"
    base_dir.mkdir(parents=True, exist_ok=True)
    today_str = get_current_ist_datetime().strftime("%Y%m%d")
    return base_dir / f"eth_liquidity_sweep_{today_str}.json"
...
class ETHLiquiditySweep:
    ...
    def evaluate_signals(self, df: pd.DataFrame, df_4h: Optional[pd.DataFrame] = None):
        ...
        # Triple 4H Trend Alignment (4H 20 > 50 EMA and 1H Close > 4H 100 EMA)
        macro_bull = True
        macro_bear = True
        curr_4h_ema20 = 0.0
        curr_4h_ema50 = 0.0
        ...
        if df_4h is not None and len(df_4h) >= 30:
            c4 = df_4h['close']
            ema20_4h = c4.ewm(span=EMA_FAST_SPAN, adjust=False).mean()
            ema50_4h = c4.ewm(span=EMA_MID_SPAN, adjust=False).mean()
            ema100_4h = c4.ewm(span=EMA_SLOW_SPAN, adjust=False).mean()
            curr_4h_ema20 = float(ema20_4h.iloc[-1])
            curr_4h_ema50 = float(ema50_4h.iloc[-1])
            curr_4h_ema100 = float(ema100_4h.iloc[-1])
            macro_bull = (curr_4h_ema20 > curr_4h_ema50) and (curr_c > curr_4h_ema100)
            macro_bear = (curr_4h_ema20 < curr_4h_ema50) and (curr_c < curr_4h_ema100)

        # Rolling 2-Day (48h) Swing High / Low strictly prior to evaluation candle
        ...
        logger.info(f"[HEARTBEAT] Spot=${latest_spot:.2f} (1H Candle Close=${curr_c:.2f}) | 4H 20 EMA=${curr_4h_ema20:.2f} | 4H 50 EMA=${curr_4h_ema50:.2f} | 4H 100 EMA=${curr_4h_ema100:.2f} [{regime}] | 48h Swing High=${swing_high:.2f} | Swing Low=${swing_low:.2f}")
        ...
        # 1. Bullish Liquidity Sweep (Sweeping Key Swing Low + 35% Rejection Wick + Triple 4H Bullish Trend)
        ...
        if (curr_l < swing_low and curr_c > swing_low and 
            ...
            entry_price = round(curr_c, 2)
            stop_loss = round(curr_l - 1.0, 2) # Hard buffer below sweep wick extreme
            risk_dist = entry_price - stop_loss
            risk_pct = risk_dist / entry_price
            if MIN_RISK_DIST_PCT <= risk_pct <= 0.035:
                risk_usd = capital_usd * RISK_PER_TRADE_PCT
                loss_per_contract = risk_dist * 0.01 # 0.01 multiplier for ETH
                raw_lots = int(risk_usd / loss_per_contract)
                max_lots = int((capital_usd * MAX_LEVERAGE) / (entry_price * 0.01))
                lots = max(2, min(raw_lots, max_lots))

                target1_p = round(entry_price + (1.0 * risk_dist), 2)
                target2_p = round(entry_price + (TARGET_RR * risk_dist), 2)
                target3_p = round(entry_price + (TARGET_RR * risk_dist), 2)

                logger.info("=" * 80)
                logger.info(f"🚀 INSTITUTIONAL BULLISH ETH SFP DETECTED ({EXECUTION_ORDER_TYPE})!")
                logger.info(f"  • Swept Swing Low : ${swing_low:.2f} (Low: ${curr_l:.2f} | Depth: {sweep_depth_pct*100:.2f}%)")
                logger.info(f"  • Triple 4H Trend : 20 EMA (${curr_4h_ema20:.2f}) > 50 EMA (${curr_4h_ema50:.2f}) & Close > 100 EMA (${curr_4h_ema100:.2f})")
                ...
                logger.info(f"  • Targets         : TP1=${target1_p:.2f} (1.0R - Lock BE) | TP2=${target2_p:.2f} ({TARGET_RR}R)")
        ...
        # 2. Bearish Liquidity Sweep (Sweeping Key Swing High + 35% Rejection Wick + Triple 4H Bearish Trend)
        ...
        if (curr_h > swing_high and curr_c < swing_high and 
            ...
            entry_price = round(curr_c, 2)
            stop_loss = round(curr_h + 1.0, 2) # Hard buffer above sweep wick extreme
            ...
            if MIN_RISK_DIST_PCT <= risk_pct <= 0.035:
                ...
                logger.info("=" * 80)
                logger.info(f"🔻 INSTITUTIONAL BEARISH ETH SFP DETECTED ({EXECUTION_ORDER_TYPE})!")
    ...
    def run_loop(self):
        logger.info(f"Starting {STRATEGY_NAME} Monitoring Loop on 1H Candles (Triple 4H Trend Alignment)...")
        while True:
            try:
    ...
    def run_test(self, place_test_order: bool = False):
        ...
        if place_test_order:
            ...
            curr_c = float(df.iloc[-1]["close"]) if df is not None and len(df) > 0 else 2500.0
            test_entry = round(curr_c * 0.90, 2) # Limit buy 10% below market
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ETH Liquidity Sweep Perpetual Strategy")
    ...
    strategy = ETHLiquiditySweep(host=host, api_key=api_key, capital_inr=args.capital, dry_run=args.dry_run)
