...
SWING_LOOKBACK_BARS = int(os.getenv("ETH_SWING_LOOKBACK", "36"))     # 36 bars = 9 hours rolling swing (Filters chop)
MIN_REJECTION_WICK_RATIO = float(os.getenv("ETH_WICK_RATIO", "0.35")) # 35% wick rejection minimum
MAX_LEVERAGE = float(os.getenv("ETH_MAX_LEVERAGE", "12.0"))           # Strict 12x leverage cap to eliminate high brokerage
MIN_RISK_DIST_PCT = float(os.getenv("ETH_MIN_RISK_DIST", "0.0025"))   # 0.25% minimum stop distance to reject micro-wicks
MAX_HOLD_BARS = int(os.getenv("ETH_MAX_HOLD_BARS", "28"))             # 28 bars = 7 hours max intraday hold
...
RISK_PER_TRADE_PCT = float(os.getenv("ETH_RISK_PER_TRADE", "0.06"))   # Risk 6% of capital per trade with 12x cap
...
class ETHLiquiditySweep:
    ...
    def evaluate_signals(self, df: pd.DataFrame):
        ...
        # Calculate 200 EMA Macro Trend Filter on 15m
        ...
        if (curr_l < swing_low and curr_c > swing_low and 
            ...
            if MIN_RISK_DIST_PCT < risk_pct < 0.018:
                ...
                logger.info("=" * 80)
                logger.info(f"🚀 INSTITUTIONAL BULLISH ETH SFP DETECTED (LIMIT MAKER ORDER)!")
        ...
        if (curr_h > swing_high and curr_c < swing_high and 
            ...
            if MIN_RISK_DIST_PCT < risk_pct < 0.018:
                ...
                logger.info("=" * 80)
                logger.info(f"🔻 INSTITUTIONAL BEARISH ETH SFP DETECTED (LIMIT MAKER ORDER)!")
